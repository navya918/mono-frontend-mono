    ...
    useEffect(() => {
        ...
        const fetchRemainingLeaveDays = async (employeeId, leaveType, leaveStartDate, leaveEndDate) => {
            ...
            try {
                const token = localStorage.getItem('token')
                console.log(token)
                const response = await axios.get('http://localhost:8085/apis/employees/remaining-leaves', {
                    method:'GET',
                    headers:{
                      'Authorization' : `Bearer ${token}`,
                      'Content-Type' : 'application/json'
...
requiredFields.forEach(field => {
    console.log(`${field}: ${formData[field]}`);
        ...
        try {
            ...
            const url = isEditing
                ? `http://localhost:8085/apis/employees/update/${formData.id}`
                : `http://localhost:8085/apis/employees/submit`;
   ...
   //const md = formData.medicalDocument
    return (
        <div className="flex items-center justify-center min-h-screen py-4 px-4 sm:px-6 md:px-8">
